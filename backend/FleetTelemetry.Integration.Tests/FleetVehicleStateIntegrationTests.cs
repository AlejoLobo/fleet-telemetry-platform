...
public class FleetVehicleStateIntegrationTests : IAsyncLifetime
{
    ...
    public async Task ProcessAsync_actualiza_read_model_fleet_vehicle_state()
    {
        ...
        var telemetryEvent = CreateEvent(
            DeviceIdTestHelper.CreateDeterministicGuid("VH-READ-001"),
    ...
    public async Task ProcessAsync_upsert_reemplaza_estado_existente_del_mismo_vehiculo()
    {
        ...
        var deviceId = DeviceIdTestHelper.CreateDeterministicGuid("VH-UPSERT-001");
    ...
    public async Task ProcessAsync_evento_fuera_de_orden_no_revierte_estado()
    {
        ...
        var deviceId = DeviceIdTestHelper.CreateDeterministicGuid("VH-OOO-001");
        var deviceIdStorage = deviceId.ToString("D");
    ...
    public async Task ProcessAsync_mismo_timestamp_mayor_eventId_gana_desempate()
    {
        ...
        var deviceId = DeviceIdTestHelper.CreateDeterministicGuid("VH-TIE-001");
        var deviceIdStorage = deviceId.ToString("D");
        ...
        var first = CreateEvent(deviceId, timestamp, speedKmh: 40, latitude: 4.61, longitude: -74.09, eventId: lowerEventId);
        var second = CreateEvent(deviceId, timestamp, speedKmh: 42, latitude: 4.62, longitude: -74.08, eventId: higherEventId);
    ...
    public async Task DatabaseInitializer_backfill_reconstruye_fleet_vehicle_state_desde_telemetry()
    {
        ...
        await SeedTelemetryOnlyAsync(
            (DeviceIdTestHelper.CreateDeterministicGuid("VH-BF-001"), baseTime.AddMinutes(10), Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"), 30, 4.60, -74.10),
            (DeviceIdTestHelper.CreateDeterministicGuid("VH-BF-001"), baseTime.AddMinutes(30), Guid.Parse("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"), 50, 4.65, -74.08),
            (DeviceIdTestHelper.CreateDeterministicGuid("VH-BF-002"), baseTime.AddMinutes(20), Guid.Parse("cccccccc-cccc-cccc-cccc-cccccccccccc"), 15, 4.55, -74.12));
    ...
    public async Task ProcessAsync_duplicado_hace_rollback_y_no_corrompe_estado()
    {
        ...
        var telemetryEvent = CreateEvent(
            DeviceIdTestHelper.CreateDeterministicGuid("VH-ROLL-001"),
    ...
    public async Task GetVehicleStatusAsync_retorna_estado_desde_read_model()
    {
        ...
        var telemetryEvent = CreateEvent(
            DeviceIdTestHelper.CreateDeterministicGuid("VH-STATUS-001"),
            now.AddMinutes(-2),
            speedKmh: 45,
            latitude: 4.64,
            longitude: -74.09);
        ...
        Assert.Equal(telemetryEvent.DeviceId, status.DeviceId);
    ...
    public async Task GetVehicleStatusAsync_calcula_heading_con_punto_previo()
    {
        ...
        var deviceId = DeviceIdTestHelper.CreateDeterministicGuid("VH-HDG-001");
        var deviceIdStorage = deviceId.ToString("D");
        var first = CreateEvent(
            deviceId,
            new DateTimeOffset(2026, 7, 10, 10, 0, 0, TimeSpan.Zero),
